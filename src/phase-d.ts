...
type GroupSpec = {
  ...
  status?: Status;
  cpcBidMicros?: number;
  moves: Array<{ keywordId: string; sourceAdGroupId: string }>;
  rsa?: {
    finalUrl: string;
    headlines: string[];
    descriptions: string[];
...
type Plan = {
  ...
  campaignId: string;
  groups: GroupSpec[];
  leadFormBiddable?: boolean;
  expectedCampaignStatus: string;
  expectedGroups: Array<{ id: string; name: string; status: string }>;
  expectedKeywords: Array<{ resourceName: string; status: string; sourceAdGroupId: string }>;
  expectedLeadGoalBiddable?: boolean;
  expiresAt: number;
};
...
function decode(token: string): Plan {
  ...
  let plan: Plan;
  try { plan = JSON.parse(Buffer.from(payload, 'base64url').toString('utf8')) as Plan; } catch { throw new Error('Invalid confirmation token.'); }
...
const escapeGaql = (value: string) => value.replace(/\\/g, '\\\\').replace(/'/g, "\\'");
...
function validateRsa(rsa: NonNullable<GroupSpec['rsa']>): void {
  let url: URL;
  try { url = new URL(rsa.finalUrl); } catch { throw new Error('RSA finalUrl must be an absolute URL.'); }
...
async function validateReferences(campaignId: string, groups: GroupSpec[], leadFormBiddable?: boolean) {
  ...
  for (const group of groups) {
    ...
    if (existingNames.has(name)) throw new Error(`Destination ad group '${group.name.trim()}' already exists in campaign ${campaignId}.`);
    if (plannedNames.has(name)) throw new Error(`Destination ad group '${group.name.trim()}' is duplicated in the restructure.`);
    plannedNames.add(name);
  }

  const expectedKeywords: Plan['expectedKeywords'] = [];
  const seenSources = new Set<string>();
  for (const group of groups) {
    for (const move of group.moves) {
      const sourceKey = `${move.sourceAdGroupId}:${move.keywordId}`;
      if (seenSources.has(sourceKey)) throw new Error(`Keyword ${move.keywordId} from ad group ${move.sourceAdGroupId} appears more than once.`);
      ...
      if (!criterion?.resource_name) throw new Error(`Positive keyword ${move.keywordId} was not found in source ad group ${move.sourceAdGroupId}.`);
      if (!row?.keyword?.text || !row?.keyword?.match_type) throw new Error(`Keyword ${move.keywordId} has incomplete keyword data.`);
      expectedKeywords.push({ resourceName: String(criterion.resource_name), status: String(criterion.status), sourceAdGroupId: move.sourceAdGroupId });
      move.__loaded = { resourceName: String(criterion.resource_name), text: String(row.keyword.text), matchType: String(row.keyword.match_type), status: String(criterion.status) } as never;
    }
  }

  let leadGoal: { resourceName: string; biddable: boolean } | undefined;
  if (leadFormBiddable !== undefined) {
    const [row] = await customer.query(`SELECT campaign_conversion_goal.resource_name, campaign_conversion_goal.biddable FROM campaign_conversion_goal WHERE campaign.id = ${campaignId} AND campaign_conversion_goal.category = SUBMIT_LEAD_FORM AND campaign_conversion_goal.origin = GOOGLE_HOSTED LIMIT 1`);
    if (!row?.campaign_conversion_goal?.resource_name) throw new Error(`Google-hosted lead-form conversion goal was not found for campaign ${campaignId}.`);
    ...
    if (leadGoal.biddable === leadFormBiddable) throw new Error(`Google-hosted lead-form conversion goal is already ${leadFormBiddable ? 'enabled' : 'disabled'} for bidding.`);
  }
  return { campaign, existingGroups, expectedKeywords, leadGoal };
}

function loadedMove(group: GroupSpec, move: GroupSpec['moves'][number]) {
  const data = (move as typeof move & { __loaded?: { resourceName: string; text: string; matchType: string; status: string } }).__loaded;
  if (!data) throw new Error(`Keyword ${move.keywordId} was not loaded during validation.`);
  return data;
}

export async function validateCampaignRestructure(input: { campaignId: string; groups: GroupSpec[]; leadFormBiddable?: boolean }) {
  validateSpec(input.campaignId, input.groups);
  const loaded = await validateReferences(input.campaignId, input.groups, input.leadFormBiddable);
  return {
    valid: true as const,
    campaign: { id: loaded.campaign.id, name: loaded.campaign.name, status: loaded.campaign.status },
    destinationAdGroups: input.groups.map((group, index) => ({ temporaryResourceName: `customers/${getCustomer().credentials.customer_id}/adGroups/-${index + 1}`, name: group.name.trim(), status: group.status ?? 'ENABLED', moves: group.moves.map((move) => ({ keywordId: move.keywordId, sourceAdGroupId: move.sourceAdGroupId, text: loadedMove(group, move).text, matchType: loadedMove(group, move).matchType, status: loadedMove(group, move).status })), rsa: group.rsa ? { finalUrl: group.rsa.finalUrl, headlines: group.rsa.headlines, descriptions: group.rsa.descriptions, path1: group.rsa.path1, path2: group.rsa.path2, status: group.rsa.status ?? 'ENABLED' } : undefined })),
...
export async function previewCampaignRestructure(input: { campaignId: string; groups: GroupSpec[]; leadFormBiddable?: boolean }) {
  const validation = await validateCampaignRestructure(input);
  ...
  const plan: Plan = {
    ...
    groups: input.groups,
    leadFormBiddable: input.leadFormBiddable,
    expectedCampaignStatus: String(loaded.campaign.status),
    expectedGroups: loaded.existingGroups.map((row) => ({ id: String(row.ad_group.id), name: String(row.ad_group.name), status: String(row.ad_group.status) })),
    expectedKeywords: loaded.expectedKeywords,
    expectedLeadGoalBiddable: loaded.leadGoal?.biddable,
    expiresAt,
  };
...
export async function applyCampaignRestructure(token: string) {
  ...
  const current = await validateReferences(plan.campaignId, plan.groups, plan.leadFormBiddable);
  ...
  const destinationResources: string[] = [];
  for (let index = 0; index < plan.groups.length; index++) {
    ...
    destinationResources.push(destination);
  ...
  for (let index = 0; index < plan.groups.length; index++) {
    const group = plan.groups[index];
    const destination = destinationResources[index];
    for (const move of group.moves) {
      const data = loadedMove(group, move);
      operations.push({ entity: 'ad_group_criterion', operation: 'create', resource: { ad_group: destination, status: data.status === 'PAUSED' ? enums.AdGroupCriterionStatus.PAUSED : enums.AdGroupCriterionStatus.ENABLED, keyword: { text: data.text, match_type: data.matchType } } });
      operations.push({ entity: 'ad_group_criterion', operation: 'remove', resource: { resource_name: data.resourceName } });
    }
    if (group.rsa) {
      const rsa = { headlines: group.rsa.headlines.map((text) => ({ text: text.trim() })), descriptions: group.rsa.descriptions.map((text) => ({ text: text.trim() })), ...(group.rsa.path1 !== undefined ? { path1: group.rsa.path1.trim() } : {}), ...(group.rsa.path2 !== undefined ? { path2: group.rsa.path2.trim() } : {}) };
      operations.push({ entity: 'ad_group_ad', operation: 'create', resource: { ad_group: destination, status: group.rsa.status === 'PAUSED' ? enums.AdGroupAdStatus.PAUSED : enums.AdGroupAdStatus.ENABLED, ad: { final_urls: [group.rsa.finalUrl], responsive_search_ad: rsa } } });
    }
  }
  ...
  try {
    const result = await customer.mutateResources(operations as never[], { partial_failure: false });
    return { applied: true, atomic: true, campaignId: plan.campaignId, createdAdGroups: destinationResources, movedKeywords: plan.groups.flatMap((group) => group.moves.map((move) => ({ keywordId: move.keywordId, sourceAdGroupId: move.sourceAdGroupId, destinationAdGroup: group.name.trim() }))), createdRsaCount: plan.groups.filter((group) => group.rsa).length, leadFormBiddable: plan.leadFormBiddable, result };
  } catch (error) {
