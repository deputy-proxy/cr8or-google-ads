...
async function leadGoal(campaignId: string) {
  const loadedCampaign = await campaign(campaignId);
  const [goalRow] = await getCustomer().query(`SELECT campaign_conversion_goal.resource_name, campaign_conversion_goal.category, campaign_conversion_goal.origin, campaign_conversion_goal.biddable FROM campaign_conversion_goal WHERE campaign.id = ${campaignId} AND campaign_conversion_goal.category = SUBMIT_LEAD_FORM AND campaign_conversion_goal.origin = GOOGLE_HOSTED LIMIT 1`);
...
export async function previewLeadFormGoalChange(campaignId: string, biddable = false) {
  const validation = await validateLeadFormGoalChange(campaignId, biddable); const expiresAt = Date.now() + TTL;
  return { ...validation, expiresAt: new Date(expiresAt).toISOString(), confirmationToken: encode({ version: 1, kind: 'lead_form_goal', customerId: getCustomer().credentials.customer_id, campaignId, resourceName: validation.goal.resourceName, expectedBiddable: validation.goal.currentBiddable, expiresAt }) };
}

export async function applyLeadFormGoalChange(token: string) {
  const plan = decode(token); if (plan.kind !== 'lead_form_goal') throw new Error('Confirmation token is not for a lead-form goal change.');
  const loaded = await leadGoal(plan.campaignId); if (String(loaded.goal.resource_name) !== plan.resourceName || Boolean(loaded.goal.biddable) !== plan.expectedBiddable) throw new Error('Google-hosted lead-form conversion goal changed since the preview. Generate a new preview.');
  try { const result = await getCustomer().mutateResources([{ entity: 'campaign_conversion_goal', operation: 'update', resource: { resource_name: plan.resourceName, biddable: !plan.expectedBiddable } }] as never[], { partial_failure: false }); return { applied: true, campaignId: plan.campaignId, goalResourceName: plan.resourceName, biddable: !plan.expectedBiddable, result }; } catch (error) { throw new Error(`Google Ads mutation failed: ${json(error)}`); }
}

function validateRsaSpec(spec: RsaSpec) {
  assertId(spec.campaignId, 'campaignId'); assertId(spec.adGroupId, 'adGroupId');
  let url: URL; try { url = new URL(spec.finalUrl); } catch { throw new Error('finalUrl must be an absolute URL.'); }
  ...
  if (spec.headlines.some((text) => !text.trim() || text.length > HEADLINE_MAX)) throw new Error(`Each headline must be non-empty and at most ${HEADLINE_MAX} characters.`);
  if (spec.descriptions.some((text) => !text.trim() || text.length > DESCRIPTION_MAX)) throw new Error(`Each description must be non-empty and at most ${DESCRIPTION_MAX} characters.`);
  if (spec.path1 !== undefined && (!spec.path1.trim() || spec.path1.length > PATH_MAX)) throw new Error(`path1 must be non-empty and at most ${PATH_MAX} characters.`);
  if (spec.path2 !== undefined && (!spec.path2.trim() || spec.path2.length > PATH_MAX)) throw new Error(`path2 must be non-empty and at most ${PATH_MAX} characters.`);
...
async function validateRsaReferences(spec: RsaSpec) {
  validateRsaSpec(spec); const loaded = await adGroup(spec.campaignId, spec.adGroupId); const group = loaded.ad_group;
  if (!group || typeof group.resource_name !== 'string') throw new Error('Ad group resource name is unavailable.');
  ...
  const duplicate = rows.some((row) => { const rsa = row?.ad_group_ad?.ad?.responsive_search_ad; const urls = row?.ad_group_ad?.ad?.final_urls; if (!rsa || !Array.isArray(urls) || !urls[0]) return false; return rsaFingerprint({ campaignId: spec.campaignId, adGroupId: spec.adGroupId, finalUrl: String(urls[0]), headlines: (rsa.headlines ?? []).map((asset) => String(asset?.text ?? '')), descriptions: (rsa.descriptions ?? []).map((asset) => String(asset?.text ?? '')), path1: rsa.path1 ? String(rsa.path1) : undefined, path2: rsa.path2 ? String(rsa.path2) : undefined }) === rsaFingerprint(spec); });
  if (duplicate) throw new Error(`An identical responsive search ad already exists in ad group ${spec.adGroupId}.`);
  return { valid: true as const, campaign: { id: spec.campaignId }, adGroup: { id: group.id, name: group.name, resourceName: group.resource_name }, existingResponsiveSearchAds: rows.length, spec };
}
...
export async function applyRsaCreate(token: string) {
  const plan = decode(token); if (plan.kind !== 'rsa_create') throw new Error('Confirmation token is not for a responsive search ad creation.');
  const validation = await validateRsaReferences(plan.spec); if (validation.adGroup.resourceName !== plan.adGroupResourceName) throw new Error('The destination ad group changed since the preview. Generate a new preview.');
  const rsa = { headlines: plan.spec.headlines.map((text) => ({ text: text.trim() })), descriptions: plan.spec.descriptions.map((text) => ({ text: text.trim() })), ...(plan.spec.path1 !== undefined ? { path1: plan.spec.path1.trim() } : {}), ...(plan.spec.path2 !== undefined ? { path2: plan.spec.path2.trim() } : {}) };
  try { const result = await getCustomer().mutateResources([{ entity: 'ad_group_ad', operation: 'create', resource: { ad_group: plan.adGroupResourceName, status: plan.spec.status === 'PAUSED' ? enums.AdGroupAdStatus.PAUSED : enums.AdGroupAdStatus.ENABLED, ad: { final_urls: [plan.spec.finalUrl], responsive_search_ad: rsa } } }] as never[], { partial_failure: false }); return { applied: true, campaignId: plan.spec.campaignId, adGroupId: plan.spec.adGroupId, result }; } catch (error) { throw new Error(`Google Ads mutation failed: ${json(error)}`); }
}
