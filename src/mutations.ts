...
function decodePlan(token: string): MutationPlan {
  ...
  try { plan = JSON.parse(Buffer.from(payload, 'base64url').toString('utf8')) as MutationPlan; }
...
function validateCampaignChange(change: CampaignChange): void {
  ...
  if (change.dailyBudgetMicros < MIN_DAILY_BUDGET_MICROS || change.dailyBudgetMicros > MAX_DAILY_BUDGET_MICROS) {
...
async function loadTarget(target: MutationTarget): Promise<Snapshot> {
  ...
  if (target.type === 'campaign') {
    rows = await customer.query(`SELECT campaign.resource_name, campaign.id, campaign.name, campaign.status, campaign_budget.resource_name, campaign_budget.amount_micros FROM campaign WHERE campaign.id = ${target.id} LIMIT 1`) as Snapshot[];
  } else if (target.type === 'ad_group') {
    rows = await customer.query(`SELECT ad_group.resource_name, ad_group.id, ad_group.name, ad_group.status FROM ad_group WHERE ad_group.id = ${target.id} LIMIT 1`) as Snapshot[];
  } else {
    rows = await customer.query(`SELECT ad_group_criterion.resource_name, ad_group_criterion.criterion_id, ad_group_criterion.status, ad_group_criterion.keyword.text FROM keyword_view WHERE ad_group_criterion.criterion_id = ${target.id} LIMIT 1`) as Snapshot[];
  }
  if (!rows[0]) throw new Error(`${target.type} ${target.id} was not found or is not accessible.`);
  return rows[0];
}
function snapshotStatus(snapshot: Snapshot, type: MutationTarget['type']): string | undefined {
  return type === 'campaign' ? snapshot.campaign?.status : type === 'ad_group' ? snapshot.ad_group?.status : snapshot.ad_group_criterion?.status;
}
function snapshotResource(snapshot: Snapshot, type: MutationTarget['type']): string | undefined {
  return type === 'campaign' ? snapshot.campaign?.resource_name : type === 'ad_group' ? snapshot.ad_group?.resource_name : snapshot.ad_group_criterion?.resource_name;
}
export async function validateMutation(target: MutationTarget) {
  if (target.type === 'campaign') validateCampaignChange(target.change);
  else validateStatus(target.change.status);
  ...
  if ('dailyBudgetMicros' in target.change) {
    const currentBudget = Number(current.campaign_budget?.amount_micros ?? 0);
    if (target.change.dailyBudgetMicros === currentBudget) throw new Error('Campaign budget is already set to that amount.');
  }
  return {
    type: target.type,
    id: target.id,
    resourceName,
    current: { status, dailyBudgetMicros: Number(current.campaign_budget?.amount_micros ?? 0), name: current.campaign?.name ?? current.ad_group?.name, keywordText: current.ad_group_criterion?.keyword?.text },
...
export async function previewMutation(target: MutationTarget) {
  ...
  const plan: MutationPlan = {
    ...
    expectedStatus: snapshotStatus(current, target.type),
    expectedBudgetMicros: String(current.campaign_budget?.amount_micros ?? 0),
    expiresAt,
  };
